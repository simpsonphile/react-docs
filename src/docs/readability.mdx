...
## Components body order

```tsx
export default function Icon(props: IIconProps): React.ReactElement {
    const { color, size, name, hasStroke, strokeWidth } = props;

    const xlinkHref = `/app/svg-symbols.svg#${name}`;
    ...
    return (
        <span
            className={classNames(styles.icon, {
                ...
                [styles[`icon--stroke`]]: hasStroke,
                [styles[`icon--stroke-${strokeWidth}`]]:
                    !!strokeWidth,
